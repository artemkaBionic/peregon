(function() {
    ...
    function eventService(
        $rootScope, socketService, $location, toastr, $state, $uibModal,
        stationService) {
            ...
            if (data.progress >= 100) {
                if (service.isDeviceNotificationEnabled) {
                    var toast = toastr.info(
                        'Media has been successfully applied, you may remove the device.',
                        'Apply Media Complete', {
                            'timeOut': 0
                        }
                    );
                    socketService.once('event', function(event) {
                        if (event.name === 'device-remove' &&
                            event.data.id === data.device.id) {
                            toastr.clear(toast);
                        }
                    });
                }
        ...
        function processConnectionState(connectionState) {
            ...
            if (connectionState.isOnline) {
                ...
                service.connectionNotification = toastr.error(
        ...
        function openModal(connectionState) {
            ...
            service.modalWindow = $uibModal.open({
